...
async function sendEmail(to,otp){
  const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({
    from:process.env.OTP_FROM_EMAIL||'RKL Portal <no-reply@rkl.sa>',to:[to],subject:'RKL verification code | رمز تحقق RKL',
    html:`<div style="font-family:Arial;text-align:center;padding:30px"><img src="https://rkl.sa/public/images/logo.png" width="80"><h2>رمز التحقق</h2><p style="font-size:32px;letter-spacing:8px;font-weight:bold">${otp}</p><p>صالح لمدة 10 دقائق.</p></div>`
...
async function secured(req,res,database,roles){
  const found=await user(req,database);if(!found){reply(res,401,{message:'انتهت الجلسة أو لم يتم تسجيل الدخول.'});return null}
  if(roles&&!roles.includes(found.profile.role)){reply(res,403,{message:'ليس لديك صلاحية لتنفيذ هذا الإجراء.'});return null}
  return found
}
...
export default async function handler(req,res){
  ...
  if(required.some(key=>!process.env[key]))return reply(res,503,{message:'بوابة العملاء قيد التجهيز.'});
  const database=db(),path=route(req);
  try{
    if(req.method==='POST'&&path==='/auth/register/start'){
      const {name,company,email,consent}=req.body||{},mobile=phone(req.body?.phone);
      if(!name||!company||!email||!consent||!/^5\d{8}$/.test(mobile))return reply(res,400,{message:'تحقق من البيانات المطلوبة.'});
      ...
      if(error)throw error;await Promise.all([sendEmail(email,emailOtp),sendSms(mobile,phoneOtp)]);
      return reply(res,200,{registrationId:data.id})
    }
    if(req.method==='POST'&&path==='/auth/register/verify'){
      ...
      if(!p||new Date(p.expires_at)<new Date()||p.attempts>=5)return reply(res,400,{message:'انتهت صلاحية الرموز.'});
      if(!equal(emailCode,p.email_code_hash)||!equal(phoneCode,p.phone_code_hash)){await database.from('pending_registrations').update({attempts:p.attempts+1}).eq('id',p.id);return reply(res,400,{message:'أحد الرموز غير صحيح.'})}
      const {data:auth,error:authError}=await database.auth.admin.createUser({email:p.email,phone:`+966${p.phone}`,email_confirm:true,phone_confirm:true});if(authError)throw authError;
      ...
      const {data:profile,error:profileError}=await database.from('profiles').insert({id:auth.user.id,organization_id:org.id,full_name:p.full_name,email:p.email,phone:p.phone,role:'client_approver',email_verified_at:new Date().toISOString(),phone_verified_at:new Date().toISOString()}).select().single();if(profileError)throw profileError;
    ...
    if(req.method==='POST'&&path==='/auth/login/start'){
      ...
      const {error}=await database.auth.signInWithOtp(credentials);if(error)throw error;return reply(res,200,{message:'تم إرسال رمز الدخول.'})
    }
    if(path==='/me'){
    ...
    if(path==='/files/upload-ticket'&&req.method==='POST'){
      ...
      if(!name||!size||size>52428800)return reply(res,400,{message:'الملف غير صالح أو يتجاوز 50MB.'});
    ...
    if(path==='/files/complete'&&req.method==='POST'){const actor=await secured(req,res,database);if(!actor)return;const {data}=await database.from('documents').select('*').eq('id',req.body.fileId).eq('organization_id',actor.profile.organization_id).single();await audit(database,actor,'file_uploaded','document',data.id);return reply(res,200,{file:{...data,name:data.title,status:'تم الرفع'}})}
    if(path.match(/^\/documents\/[^/]+\/action$/)&&req.method==='POST'){
      ...
      if(!statuses[action])return reply(res,400,{message:'الإجراء غير صالح.'});const {data:doc}=await database.from('documents').select('*').eq('id',id).single();if(!doc)return reply(res,404,{message:'المستند غير موجود.'});
      if(action.startsWith('client_')&&doc.organization_id!==actor.profile.organization_id)return reply(res,403,{message:'لا تملك صلاحية المستند.'});
    ...
    if(path.startsWith('/assets/qr/')&&req.method==='GET'){const token=path.split('/').pop(),{data}=await database.from('assets').select('id,asset_code,kind,status,buildings(name,projects(name,city))').eq('qr_token',token).single();return data?reply(res,200,{asset:data}):reply(res,404,{message:'رمز المصعد غير صالح.'})}
    ...
    return reply(res,404,{message:'المسار غير موجود.'})
  }catch(error){console.error(error);return reply(res,500,{message:'حدث خطأ أثناء معالجة الطلب.'})}
}
